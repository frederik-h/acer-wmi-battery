...
package() {
  ...
  # Install other files
  post_install() {
    ln -s /usr/lib/systemd/system/acer-battery-control-gui.service /etc/systemd/system/acer-battery-control-gui.service
    systemctl enable acer-battery-control-gui.service
    systemctl start acer-battery-control-gui.service
  }
}
